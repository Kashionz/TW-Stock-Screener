...
const TWSE_PRICE_URL = "https://www.twse.com.tw/rwd/zh/afterTrading/MI_INDEX";
...
function latestRocDate(...values) {
  let latest = "";
  for (const value of values) {
    const normalized = safeString(value).trim();
    if (!/^\d{7,8}$/.test(normalized)) continue;
...
export function buildTwseMiIndexUrl(date = yyyymmddInTaipei()) {
  const url = new URL(TWSE_PRICE_URL);
  url.searchParams.set("response", "json");
  url.searchParams.set("type", "ALLBUT0999");
  url.searchParams.set("date", safeString(date));
  return url.toString();
}
...
function rocDateFromIsoDate(value) {
  const normalized = safeString(value).trim();
  const match = normalized.match(/^(\d{4})(\d{2})(\d{2})$/);
  if (!match) return "";
  const rocYear = String(Number(match[1]) - 1911).padStart(3, "0");
...
function addPriceRows(rows, data, kind) {
  for (const record of data || []) {
    ...
    if (kind === "twse") {
      ...
      rows[code].chgText = rows[code].chg == null ? normalizeChangeText(record.Change) : null;
      continue;
    }
...
export function parseTwseMiIndexPriceRows(payload) {
  const tables = Array.isArray(payload?.tables) ? payload.tables : [];
  const table = tables.find((entry) =>
    Array.isArray(entry?.fields) &&
    Array.isArray(entry?.data) &&
    entry.fields.includes("證券代號") &&
    entry.fields.includes("收盤價") &&
    entry.fields.includes("漲跌價差"),
  );
  if (!table) return [];
  ...
  const date = rocDateFromIsoDate(payload?.date);

  return table.data
    .filter((row) => Array.isArray(row) && isStock(row[codeIndex]))
    .map((row) => {
      const sign = safeString(row[signIndex]).includes("-") ? "-" : "";
      const change = safeString(row[changeIndex]).replace(/,/g, "").trim();
      return {
        Date: date,
        Code: safeString(row[codeIndex]),
        Name: safeString(row[nameIndex]),
        ClosingPrice: safeString(row[closeIndex]).replace(/,/g, "").trim(),
        Change: change ? `${sign}${change}` : "",
      };
    });
}

async function fetchTwsePriceRows(date = yyyymmddInTaipei()) {
  try {
    const payload = await fetchJson(buildTwseMiIndexUrl(date));
    return parseTwseMiIndexPriceRows(payload);
  } catch {
    return [];
  }
}
...
function buildRowMap({
  ...
  const valuationDateRoc = latestRocDate(
    twseValuation?.[0]?.Date,
    tpexValuation?.[0]?.Date,
    twsePrice?.[0]?.Date,
    tpexPrice?.[0]?.Date,
  );
